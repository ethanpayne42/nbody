module init
  implicit none
  real, parameter :: pi = 3.1415926536

contains
  subroutine initialise(x,v,m,np,maxp)
    integer, intent(in) :: maxp
    integer, intent(out) :: np
    real, intent(out) :: x(3,maxp), v(3,maxp)
    real, intent(out) :: m(maxp)
    real :: a,e,mtot,r,v0,theta,rmin
    x = 0. ! set all positions to zero
    v = 0. ! all velocities to zero
    m = 0. ! masses of all particles to zero
    np = 2
    m(1) = 1
    m(2) = 1
    e = 0.6
    rmin = 25
    theta = 60.*(pi/180.) ! inclination
    a = rmin/(1. - e)
    r = a*(1. + e) ! start at apastron rather than periastron
    mtot = m(1) + m(2)
    x(1,1) = -r*m(2)/mtot
    x(1,2) = r*m(1)/mtot
    v0 = sqrt(a*(1.-e**2)*mtot)/r
    v(2,1) = -m(2)/mtot*v0
    v(2,2) = m(1)/mtot*v0
    call add_galaxy(x,v,np,maxp,x(:,1),v(:,1),m(1),theta)
    call add_galaxy(x,v,np,maxp,x(:,2),v(:,2),m(2),theta)
  end subroutine initialise

  subroutine add_galaxy(x,v,np,maxp,x0,v0,m0,theta)
    integer, intent(in) :: maxp
    real, intent(out) :: x(3,maxp), v(3,maxp)
    integer, intent(inout) :: np
    real, intent(in) :: x0(3),v0(3)
    real, intent(in) :: m0
    integer :: nrings,nphi,j,i
    real :: dphi,dr,ri,phi,vphi,theta
    nrings = 5
    dr = 3.

    do j=1,nrings
      ri = j*dr
      nphi = 12 + 6*(j-1) ! see toomre
      vphi = sqrt(m0/ri) ! keplerian rotation
      dphi = 2.*pi/nphi
      print*,'r = ',ri,' nphi = ',nphi,' dphi = ',dphi
      do i=1,nphi
        phi = (i-1)*dphi
        np = np + 1
        if (np > maxp) stop ' np > maxp: need bigger maxp'
        x(:,np) = x0 + (/ri*cos(phi)*cos(theta),ri*sin(phi), &
                         -ri*cos(phi)*sin(theta) /)
        v(:,np) = v0 + (/-vphi*sin(phi)*cos(theta), vphi*cos(phi) , &
                          vphi*sin(phi)*sin(theta) /)
      enddo
    enddo

    print*,' setup ',np,' particles'
    end subroutine add_galaxy

end module init
